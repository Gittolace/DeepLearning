...
def generatorModel(noise_img, units_size, out_size, alpha=0.01):
    ...
    with tf.variable_scope('generator'):
        ...
        FullConnect = tf.layers.dense(noise_img, units_size)

        reLu = tf.nn.leaky_relu(FullConnect, alpha)
...
def discriminatorModel(images, units_size, alpha=0.01, reuse=False):
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        FullConnect = tf.layers.dense(images, units_size)
...
def train(mnist):
    ...
    fake_images = tf.placeholder(tf.float32, [None, 100])
    ...
    G_loss, D_loss = loss_function(real_logits, fake_logits, SMOOTH)
    ...
    with tf.Session() as session:
        ...
        for epoch in range(EPOCH):
            ...
            for batch_i in range(mnist.train.num_examples // BATCH_SIZE):
                ...
                noise_image = np.random.uniform(-1, 1, size=(BATCH_SIZE, 100))
            ...
            print('epoch:', epoch, '判别器损失:', loss_D, ' 生成器损失:', loss_G)
