...
def seq_predict_model(X, w, b, time_step_size, vector_size):
    ...
    cell = tf.contrib.rnn.DropoutWrapper(tf.nn.rnn_cell.BasicLSTMCell(num_units=7,forget_bias=1.0,state_is_tuple=True),output_keep_prob=0.6)
    outputs, _states = tf.contrib.rnn.static_rnn(cell, X, dtype=tf.float32)
